...
class Card_service extends CI_Model{
	...
	public function get_card_like_name_and_remark($name, $remark, $pageIndex, $pageSize)
	{
		$data = $this->card_model->get_card_like_name_and_remark($name, $remark);
		$num = count($data['data']);
		if( $num == 0 )
			return array(
				'code'=>1,
	...
	public function get_card_like_name($name, $pageIndex, $pageSize)
	{
		$data = $this->card_model->get_card_like_name($name);
		$num = count($data['data']);
		if( $num == 0 )
			return array(
				'code'=>1,
	...
	public function get_card_like_remark($remark, $pageIndex, $pageSize)
	{
		$data = $this->card_model->get_card_like_remark($remark);
		$num = count($data['data']);
		if( $num == 0 )
			return array(
				'code'=>1,
	...
	{
		$data = $this->card_model->get_card_by_name($name);
		$num = count($data['data']);
		if( $num != 0 )
			return array(
				'code'=>1,
	...
	{
		$result = $this->card_model->get_card_by_id($cardId);
		$tmp = $result['data'];
		if( $tmp[0]['name'] == $name )
		{	
			$data = $this->card_model->mod($cardId, $name, $bank, $card, $money, $remark, $modifyTime);
			return $data;
		}

		$result = $this->card_model->get_card_by_name($name);
